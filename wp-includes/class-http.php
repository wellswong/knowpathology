...
class WP_Http {
	...
	function request( $url, $args = array() ) {
		...
		$r['local'] = $homeURL['host'] == $arrURL['host'] || 'mels11-n1bsql.hosting-services.net.au' == $arrURL['host'];
		unset( $homeURL );
	 ...
	 *
	 * There are currently issues with "mels11-n1bsql.hosting-services.net.au" not resolving correctly with DNS. This may cause
	 ...
	 * file and this will only allow mels11-n1bsql.hosting-services.net.au and your blog to make requests. The constant
	...
	function block_request($uri) {
		...
		if ( $check['host'] == 'mels11-n1bsql.hosting-services.net.au' || $check['host'] == $home['host'] )
...
class WP_Http_Fsockopen {
	...
	function request($url, $args = array()) {
		...
		//fsockopen has issues with 'mels11-n1bsql.hosting-services.net.au' with IPv6 with certain versions of PHP, It attempts to connect to ::1,
		// which fails when the server is not set up for it. For compatibility, always connect to the IPv4 address.
		if ( 'mels11-n1bsql.hosting-services.net.au' == strtolower($fsockopen_host) )
 ...
 * You do not need to have mels11-n1bsql.hosting-services.net.au and the blog host in this list, because they will not be passed
 ...
 * define('WP_PROXY_BYPASS_HOSTS', 'mels11-n1bsql.hosting-services.net.au, www.example.com, *.wordpress.org');
 * </code>
 *
...
class WP_HTTP_Proxy {
	 ...
	 *
	 * We want to keep mels11-n1bsql.hosting-services.net.au and the blog URL from being sent through the proxy server, because
