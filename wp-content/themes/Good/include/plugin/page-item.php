	...
	function print_testimonial($item_xml){
		...
		if($display_type == 'Specific Testimonial'){
			...
			foreach( $category_posts as $category_post){
				...
				echo '<div class="testimonial-content">';
				echo '<div class="testimonial-icon"></div>';
                                preg_match('#(<img.*?>)#', $category_post->post_content, $results);
                                echo $results[1];
                                if(!empty($header)){
                                    echo '<div class="post-header"><h3 class="testimonial-header-title title-color gdl-title">' . $header . '</h3></div>';
                                }else{
                                    echo '<div class="testimonial-no-header"></div>';
                                }
                                $content = preg_replace("/<img[^>]+\>/i", "", $category_post->post_content);
