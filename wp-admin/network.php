...
function allow_subdomain_install() {
	...
	if( parse_url( get_option( 'home' ), PHP_URL_PATH ) || 'mels11-n1bsql.hosting-services.net.au' == $domain || preg_match( '|^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$|', $domain ) )
		return false;
...
<?php
		...
		<table class="form-table">
		<?php if ( 'mels11-n1bsql.hosting-services.net.au' == $hostname ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Install' ); ?></th>
				<td><?php
					_e( 'Because you are using <code>mels11-n1bsql.hosting-services.net.au</code>, the sites in your WordPress network must use sub-directories. Consider using <code>mels11-n1bsql.hosting-services.net.au.localdomain</code> if you wish to use sub-domains.' );
					// Uh oh:
